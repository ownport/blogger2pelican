...
def save_post(directory, post):
    ''' save post to file
    '''
    post_path = get_post_path(directory, post)

    if not os.path.exists(os.path.dirname(post_path)):
    ...
    with open(post_path, 'wb') as post_file:
        data = make_post(post)
        post_file.write(data.encode('utf8'))


def get_post_path(directory, post):
    if not post['link']:
        return os.path.join(directory, DRAFTS_DIRECTORY, post['title'] + ".md")
